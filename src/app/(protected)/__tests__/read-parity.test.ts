...
async function seed(pg: PGlite) {
  await pg.exec(`
    ...
    insert into locations (id, warehouse_id, code) values
      ('${L1}', '${W1}', 'A-01'),
      ('${L2}', '${W1}', 'A-02'),
      ('${L3}', '${W2}', 'B-01');

    insert into categories (id, name, slug) values
      ('${C1}', 'Fasteners', 'fasteners'),
      ('${C2}', 'Tools', 'tools'),
      ('${C3}', 'Consumables', 'consumables');

    insert into suppliers (id, name) values
      ('${S1}', 'Northwind'),
      ('${S2}', 'Southwind');

    insert into products (id, sku, name, reorder_point, cost_price, is_active) values
      ('${P1}', 'A-100', 'Hex bolt',    10, '2.50', true),
      ('${P2}', 'A-200', 'Wing nut',     5, '1.25', true),
      ('${P3}', 'A-300', 'Torque key',   0, '9.00', true),
      ('${P4}', 'A-400', 'Retired shim', 0, '0.50', false);

    insert into stock_movements (product_id, location_id, type, reason, quantity, unit_cost, created_by) values
      ('${P1}', '${L1}', 'stock_in',  'purchase',   100, '2.50', '${ADMIN}'),
      ('${P1}', '${L1}', 'stock_out', 'sale',       -95, null,   '${STAFF}'),
      ('${P2}', '${L2}', 'stock_in',  'purchase',    20, '1.25', '${ADMIN}'),
      ('${P2}', '${L2}', 'stock_out', 'sale',       -18, null,   '${STAFF}'),
      ('${P3}', '${L3}', 'stock_in',  'purchase',     4, '9.00', '${ADMIN}'),
      ('${P3}', '${L3}', 'adjustment','count_correction',  1, null,   '${ADMIN}');

    insert into purchase_orders (id, po_number, supplier_id, warehouse_id, status, created_by) values
      ('${PO1}', 'PO-2026-001', '${S1}', '${W1}', 'draft', '${ADMIN}'),
      ('${PO2}', 'PO-2026-002', '${S2}', '${W2}', 'sent',  '${ADMIN}');

    insert into po_lines (po_id, product_id, quantity_ordered, quantity_received, unit_cost) values
      ('${PO1}', '${P1}', 50, 0, '2.40'),
      ('${PO1}', '${P2}', 30, 0, '1.20'),
      ('${PO2}', '${P3}', 10, 0, '8.80');

    insert into audit_log (user_id, action, resource_type, resource_id) values
      ('${ADMIN}', 'create', 'product', '${P1}'),
      ('${STAFF}', 'update', 'product', '${P2}');
